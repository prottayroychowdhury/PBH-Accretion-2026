import numpy as np
from .constants import G, c, k, pi, m_p, m_e, M_sun, sigma_T, alpha
from .history import x_e_pbh, rho_b
from .bondi import tau, r_B, v_B, lambda_acc, v_L_rms
...
def F(Y): return Y * (1 + Y / 0.27)**(-1/3)
def T_S(M, z, mode, v_rel=0): return m_e * c**2 / k * F(Y_S(M, z, mode, v_rel))
...
def Mdot(M, z, v_rel=0): return 4 * pi * rho_b(z) * r_B(M, z, v_rel)**2 * v_B(z, v_rel) * lambda_acc(M, z, v_rel)
def L_Edd(M): return 4 * pi * G * M * M_sun * m_p * c / sigma_T
def mdot(M, z, v_rel=0): return Mdot(M, z, v_rel) * c**2 / L_Edd(M)
def L(M, z, mode, v_rel=0): return epsilon_over_mdot(M, z, mode, v_rel) * mdot(M, z, v_rel) * Mdot(M, z, v_rel) * c**2
